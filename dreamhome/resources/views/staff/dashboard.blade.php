<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            {{-- PDF ACTION MOVED TO MAIN HEADER --}}
    ...
    <div class="py-12 bg-[#F3F4F6] min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="space-y-12 mb-12">
                ...
                {{-- 3. VIEWING FEEDBACK (Auto-Hides on Submit) --}}
                <div>
                    <div class="flex items-center gap-2 mb-6 px-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="bg-white rounded-[2.5rem] p-6 shadow-sm border border-white flex flex-col gap-6">
                            ...
                            <div class="space-y-2">
                                <div class="bg-gray-50 p-3 rounded-2xl border border-gray-100"><p class="text-[8px] font-black text-gray-400 uppercase mb-1">Client / Property</p><p class="text-xs font-bold">{{ $viewing->r_fname }} {{ $viewing->r_lname }} — {{ $viewing->street }}</p></div>
                            </div>
                            <form action="{{ route('staff.viewings.feedback') }}" method="POST">
                                @csrf
                                <input type="hidden" name="viewingid" value="{{ $viewing->viewingid }}">
                                <textarea name="comment" rows="2" placeholder="Record feedback here..." class="w-full bg-gray-50 border-gray-100 rounded-2xl text-xs focus:ring-[#853953] p-3"></textarea>
                                <button type="submit" class="mt-3 w-full bg-[#853953] text-white py-3 rounded-xl font-black text-[9px] uppercase tracking-widest shadow-lg shadow-pink-100 hover:bg-pink-900">Submit Feedback</button>
                            </form>
                        </div>
                        @empty
                        <div class="md:col-span-2 bg-white rounded-[2.5rem] p-10 text-center border-dashed border-gray-200 border-2"><p class="text-gray-400 font-bold text-xs uppercase">No pending feedback</p></div>
                        @endforelse
                    </div>
                </div>

                {{-- 4. INSPECTIONS & LEASES --}}
                    ...
                    <div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="bg-white rounded-[2rem] p-6 shadow-sm border border-white flex flex-col gap-4">
                                ...
                                <div class="bg-gray-50 p-4 rounded-2xl border border-gray-100"><p class="text-[8px] font-black text-gray-400 uppercase mb-1">Target Property</p><p class="text-xs font-bold">{{ $ins->street }}, {{ $ins->city }}</p></div>
                            </div>
                            @endforeach
                        </div>
                    </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="bg-white rounded-[2rem] p-6 shadow-sm border border-white flex flex-col gap-4">
                                ...
                                <div class="flex items-center gap-4 bg-gray-50 p-3 rounded-2xl border border-gray-100">
                                    <div class="flex-1"><p class="text-[8px] font-black text-gray-400 uppercase">Renter</p><p class="text-[10px] font-bold">{{ $lease->r_fname }} {{ $lease->r_lname }}</p></div>
                                    <div class="flex-1 text-right"><p class="text-[8px] font-black text-gray-400 uppercase">Expires</p><p class="text-[10px] font-bold text-gray-600">{{ \Carbon\Carbon::parse($lease->enddate)->format('M d, Y') }}</p></div>
                                </div>
                            </div>
                            @endforeach
                        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div class="bg-white rounded-[2.5rem] shadow-sm border border-white p-8"><h5 class="text-xl font-black text-gray-900 tracking-tighter mb-6">{{ $chartLabel }}</h5><div id="line-chart"></div></div>
                <div class="bg-white rounded-[2.5rem] shadow-sm border border-white p-8"><h5 class="text-xl font-black text-gray-900 tracking-tighter mb-6">Inventory Mix</h5><div class="py-2" id="pie-chart"></div></div>
            </div>
        </div>
    </div>
