        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                <div>
                    ...
                    <p class="text-sm font-bold text-emerald-600 mt-1 uppercase tracking-widest">Master Agreement Overview</p>
                </div>
                <div class="flex items-center gap-3">
                    {{-- Matching the Emerald button from your client-side --}}
                    <a href="{{ route('staff.leases.create') }}" 
                       class="inline-flex items-center px-6 py-3 bg-emerald-600 text-white text-xs font-black uppercase tracking-widest rounded-xl hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-200/50 active:scale-95">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 ring-1 ring-emerald-50">
                    <p class="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Paid This Month</p>
                    <p class="text-3xl font-black text-emerald-600 mt-1">{{ $leases->where('is_paid_this_month', true)->count() }}</p>
                </div>
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                ...
                <form action="{{ route('staff.leases.index') }}" method="GET" class="p-4 flex flex-col md:flex-row gap-4">
                    <div class="relative flex-1">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                               class="block w-full pl-11 pr-4 py-3 bg-slate-50 border-none rounded-2xl text-sm font-bold placeholder-slate-400 focus:ring-2 focus:ring-emerald-500 transition-all"
                               placeholder="Search by Lease ID, Renter, or Property...">
                    </div>
                    <button type="submit" class="px-8 py-3 bg-slate-800 text-white text-xs font-black uppercase tracking-widest rounded-2xl hover:bg-slate-900 transition-all shadow-md">
                        Filter Results
                    </button>
                </form>
            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-50">
                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors group">
                                <td class="px-6 py-5">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center font-black text-emerald-600 text-xs">
                                ...
                                <td class="px-6 py-5 text-center">
                                    @if($lease->is_paid_this_month)
                                        <span class="inline-flex items-center px-3 py-1 rounded-full bg-emerald-100 text-emerald-700 text-[10px] font-black uppercase tracking-widest">
                                            Current
                                        </span>
                                    @else
                                ...
                                <td class="px-6 py-5 text-right">
                                    <a href="{{ route('staff.leases.show', $lease->leaseno) }}" 
                                       class="inline-flex p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all">
