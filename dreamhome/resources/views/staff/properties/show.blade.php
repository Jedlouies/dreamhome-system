<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            {{-- Navigation logic adjusted based on position --}}
            <a href="{{ $backRoute }}" class="text-sm font-black uppercase tracking-widest text-gray-400 hover:text-[#853953] transition-colors flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 19l-7-7 7-7"/></svg>
                {{ $isRegular ? __('Back to Dashboard') : __('Back to Properties') }}
            </a>

            {{-- Actions limited to Managers/Non-Regular staff --}}
            @if(!$isRegular)
                <a href="{{ route('staff.properties.edit', $property->propertyno) }}" class="bg-[#853953] text-white px-6 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-[#6e2e44] transition-all shadow-md shadow-[#853953]/10">
                    Edit Property
                </a>
            @endif
        </div>
